...
{
  if (r == 0) return 1;
  return (fact[n] * inverse_mod_q(fact[r], p) % p * inverse_mod_q(fact[n - r], p) % p) % p;
}
...
function Dixon(n)
{
    ...
    for(var i = start; i < n; i++)
    {
      ...
      for(var j = 0; j < len; j++)
      {
        var lhs = parseInt(pow(i,2))% n;
        var rhs = parseInt(pow(base[j],2)) % n;
    ...
    for(var i = 0; i < len; i++){
        var factor = euclidean(pairs[i][0] - pairs[i][1], n);
...
module.exports.Dixon(n)
{
  return Dixon(n);
}

module.exports.PollardRho(n)
{
  return PollardRho(n);
}

module.exports.nCrModPFermat(n, r, p)
{
